...
Tests applied maximum torque to knee motors in adherence with their torque-speed curve, while hip joints followed a control law (equation \ref{eq:friction_cone_limit}) to prevent slipping. This control uses the force-torque mapping from section \ref{sec:force_torque_mapping} and friction theory from section \ref{sec:contact_friction}. Though equation \ref{eq:force_torque_mapping} assumes equilibrium, minimal slipping occurred in practice.

TODO: WHAT DOES THIS MEAN? Equation \ref
{eq:force_torque_mapping} is derived with an assumption of zero velocity/equilibria (TODO), an assumption that is obviously not valid for this dynamic jumping scenario. Despite this, very little slipping is observed in practice. 
